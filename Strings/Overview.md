...
- *Theorem 1*: Let $k$ be the sparsity of the string, and assuming $1 - p = \Omega( k^{-1/2} \log(n) )$, then $m = \exp\{ O( (k/(1-p))^{1/3} \log^{2/3}(n) ) \}$ traces will allow recovery with high probability.
